...
export class Client {
  ...
  public async getProduct(productCode: ProductCode): Promise<null> {
    const headers = new Headers();
    headers.set('Authorization', this.getAuthorizationHeader());
    const result = await fetch(`${this.#baseUrl}/v1/products/${productCode}`, { headers });
  ...
  public async listTarrifCharges(productCode: ProductCode, tariffCode: TariffCode, params?: GetTariffChargesParams): Promise<TariffCharge[]> {
    ...
    const request = `${this.#baseUrl}/v1/products/${productCode}/electricity-tariffs/${tariffCode}/standard-unit-rates/?${new URLSearchParams(urlParams).toString()}`;

    const headers = new Headers();
    headers.set('Authorization', this.getAuthorizationHeader());
    const result = await fetch(request, { headers });

    return (await result.json()).results.map((tariff) => ({
  ...
  public async getMeterConsumption(mpan: MeterPointAdministrationNumber, serialNumber: MeterSerialNumber, params?: GetMeterConsumptionParams): Promise<MeterConsumption[]> {
    const urlParams: GetTariffChargesUrlParams = {};
    if (params?.periodFrom) {
      urlParams['period_from'] = params.periodFrom.toISOString();
    }
