...
class ApiTariffChargeRepository implements TariffChargeRepository {
  constructor(private api: Client, private productCode: ProductCode, private tariffCode: TariffCode) { }
  ...
  async getMostRecentDay(): Promise<Date> {
    const result = await this.api.listTarrifCharges(this.productCode, this.tariffCode, {pageSize: 1});
...
export const createApiTariffChargeRepository = (api: Client, productCode: ProductCode, tariffCode: TariffCode) => new ApiTariffChargeRepository(api, productCode, tariffCode);
